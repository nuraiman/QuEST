...
void applyModExpOracle(Qureg& qureg, int m, qInt a, qInt N) {
    for (qInt i = 0L; i < (1L<<m); ++i) {
...
qInt periodUsingCFE(qInt y, qInt M, qInt a, qInt N) {
    qInt r = 0;
    // z = y/M
    // In theory, we know that y < M, therefore a0 = 0
    // index i starting from 0.
    qInt hi_1 = 0, hi_2 = 1, ki_1 = 1, ki_2 = 0; // initial conditions, given that a0 = 0
    qInt zi_nom, zi_denom; // inverted parameter, zi = ai + bi, with integer ai and bi < 1
    // std::cout << " z = " << y << "/" << M << std::endl;
    qInt ai, hi, ki; // The convergents of z are of the form hi/ki
    qInt bi_nom = y, bi_denom = M;
    while (true){
...
qInt PeriodFinding(QuESTEnv& env, qInt a, qInt N){
